...
function AnimatedSignOutButton({ onClick }: { onClick: () => void }) {
  return (
    ...
    >
      ...
      {/* Right Vine - Fern Leaf Branch (Far Right) */}
      <div className="absolute right-2.5 -top-2.5 h-[calc(100%+24px)] w-4 pointer-events-none z-10 text-[#1a1a1a]">
        <svg
                      ...
                      <div className="flex items-center gap-3 shrink-0 ml-4">
                        {team.isScored ? (
                          <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-orange-50/50 border border-orange-200/50">
                            ...
                            <span className="text-[10px] font-bold text-[#E83C00] font-mono">{team.totalScore !== null ? `${team.totalScore} / 50` : 'Scored'}</span>
                          </div>
                        ) : (
                ...
                {isEvaluating ? (
                  ...
                  <div className="flex flex-col flex-1 min-h-0">
                    ...
                    <div className="flex-1 overflow-y-auto p-5 space-y-4">
                      ...
                      {/* Notes / Feedback */}
                      <div className="bg-white border border-[#EAE4D8] p-4 rounded-2xl space-y-2">
                        <label className="text-xs font-bold text-[#1A1A1A] block">Overall Feedback</label>
                        <p className="text-[11px] text-slate-400 leading-normal">
                          Leave constructive feedback or private notes about this team's project.
                        </p>
                        <textarea
                          rows={3}
                          value={notes}
                          ...
                          className="w-full bg-[#F4ECE1]/50 border border-[#EAE4D8] rounded-xl p-3 text-xs font-medium text-slate-800 placeholder-slate-400 focus:outline-none focus:border-[#E83C00] transition-colors resize-none"
                        />
                      </div>
                    </div>

                    {/* Footer */}
                    <div className="p-4 border-t border-[#EAE4D8] shrink-0 pb-8 sm:pb-4">
                      <div className="flex items-center justify-between mb-3 px-1">
                        <span className="text-xs font-bold text-slate-600 uppercase tracking-wider">Total Score</span>
                        <span className="text-lg font-bold font-mono text-[#E83C00]">
                          {totalScore} <span className="text-xs text-slate-400">/ {maxPossibleScore}</span>
                        </span>
                      </div>
                      <button
                        onClick={handleSubmitEvaluation}
                        disabled={submitting || totalScore === 0}
                        className="w-full py-4 rounded-xl bg-[#E83C00] text-white font-bold text-sm shadow-[0_4px_15px_rgba(232,60,0,0.2)] hover:bg-[#FF4500] hover:shadow-[0_8px_25px_rgba(232,60,0,0.25)] transition-all disabled:opacity-50 disabled:pointer-events-none flex items-center justify-center gap-2"
                      >
                        {submitting ? 'Submitting...' : activeTeam.isScored ? `Update Evaluation (${totalScore} / ${maxPossibleScore})` : `Submit Evaluation (${totalScore} / ${maxPossibleScore})`}
                      </button>
                    </div>
                  </div>
                ) : (
                        ...
                        <div className="space-y-3">
                          <div className="flex justify-between items-center bg-orange-50/50 border border-orange-200 p-4 rounded-xl">
                            <div>
                              ...
                              <span className="text-xl font-mono font-bold text-[#E83C00]">{activeTeam.totalScore} / 50</span>
                              {activeTeam.notes && (
                          ...
                          {!activeTeam.isLocked && (
                            ...
                            >
                              Edit Evaluation ({activeTeam.totalScore} / 50)
                            </button>
                          )}
                        </div>
                      ) : (
